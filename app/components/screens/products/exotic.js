...
class Exotic extends Component {
  ...
  getProduct = () => {
    ...
    getVeggies({ type: 'EXOTIC' }, 'exotic', user && user.type)
  }
  ...
  openStockModal = () => {
    ...
    openStockExotic(true)
    openStockFruits(false)
    openStockFreshCut(false)
  }

  render() {
    const { exotic, cartFlag, stockFlag } = this.props;
...
function mapStateToProps(state) {
  return {
    exotic: state.products.exotic,
    productLoading:state.products.productLoading,
    stockFlag: state.orders.stockFlagExotic,
  };
}

export default connect(mapStateToProps, mapDispatchToProps)(Exotic);
